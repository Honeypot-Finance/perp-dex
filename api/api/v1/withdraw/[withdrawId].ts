...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (!withdrawId || typeof withdrawId !== 'string') {
      return sendError(res, 'VALIDATION_ERROR', 'withdrawId is required');
    }
    ...
    if (!result.success) {
      return handle404(res, result.message || 'Withdrawal not found');
    }

    return sendSuccess(res, result.data);

  } catch (error) {
    console.error('Withdraw by ID API error:', error);

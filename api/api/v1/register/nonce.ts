...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const result = await client.getRegistrationNonce();

    if (!result.success) {
      return sendError(
        res,
        result.code?.toString() || 'FETCH_FAILED',
        result.message || 'Failed to fetch registration nonce'
      );
    }

    return sendSuccess(res, result.data);
  } catch (error) {
    console.error('Registration nonce API error:', error);
